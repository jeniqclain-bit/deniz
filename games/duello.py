...
async def start_duello(msg, state, caller=None):
    q = random.choice(bilgi_data)
    await state.set_state(GS.duello)
    await state.update_data(answer=q["cevap"], game="duello")
    return await msg.answer(
        f"⚔️ Duello!\n\n{q['soru']}\n\nİlk doğru cevaplayan kazanır!",
        reply_markup=ctrl_kb()
    )

@router.message(GS.duello)
async def duello_answer(msg: Message, state: FSMContext):
    ...
    if msg.text.strip().lower() == ans.lower():
        ...
        await start_game_by_name(msg, state, "duello")
    else:
